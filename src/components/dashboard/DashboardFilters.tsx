...
interface DashboardFiltersProps {
  selectedWorklogType: string | null;
  setSelectedWorklogType: (type: string | null) => void;
  sortBy: SortOption;
  setSortBy: (sort: SortOption) => void;
  allWorklogTypes: string[];
}

export function DashboardFilters({
  selectedWorklogType,
  setSelectedWorklogType,
  sortBy,
  setSortBy,
  allWorklogTypes,
    ...
    <div className="flex items-center gap-3">
      ...
      {selectedWorklogType && (
        <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 rounded-md">
          <span className="text-sm text-indigo-700">Filter: {selectedWorklogType}</span>
          <button
            onClick={() => setSelectedWorklogType(null)}
        ...
        <Menu as="div" className="relative">
          ...
          <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="py-1">
              ...
              {allWorklogTypes.map((type: string) => (
                <Menu.Item key={type}>
                  {({ active }) => (
                    <button
                      onClick={() => setSelectedWorklogType(type)}
                      className={`${
                        active ? 'bg-gray-100 text-gray-900' : 'text-gray-700'
                      } block w-full px-4 py-2 text-left text-sm ${
                        selectedWorklogType === type ? 'bg-indigo-50 text-indigo-700' : ''
                      }`}
                    >
                      {type}
                    </button>
